	...
	if #nodes > 1 then
		for i = #nodes, 1, -1  do
			if not can_fit(self, nodes[i]) then
				local clear = grave.find_clearance(self, nodes[i], nodes, math.abs(hitbox(self)[4])*2.25)
				if clear
...
function grave.find_clearance(self, pos, max)
	local dist = 0
	while dist < max do
		dist = dist + 1
...
function grave.get_path(self, pos1, pos2)
	local path = minetest.find_path(pos1, pos2, self.view_range, 1, 1, "A*_noprefetch")
	if not path then return end
	for i = #path, 1, -1 do
		if not can_fit(self, path[i]) then
			local clear = grave.find_clearance(self, path[i], math.abs(hitbox(self)[4])*2.25)
			if clear
			and can_fit(self, clear) then
				path[i] = clear
			end
...
function grave.find_path(self, tpos)
		...
		if height_diff <= self.view_range/2 then
			...
			and vector.distance(self.old_follow_pos,follow_pos) > 2)) then
				...
				if not minetest.registered_nodes[minetest.get_node(vector.new(follow_pos.x,follow_pos.y-1,follow_pos.z)).name].walkable then
					...
					local index_table = minetest.find_nodes_in_area_under_air(min, max, grave.walkable_nodes)
				...
				local path = grave.get_path(self, acute_pos, follow_pos)
